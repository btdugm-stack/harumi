// Render halaman utama — padanan index.php versi PHP.
// Menu + konfigurasi di-inject saat request, sama seperti PHP server-side render.
...
function productImage(item) {
  ...
  if (item.category === 'Non Coffee') {
    if (/choco|velvet/i.test(item.name)) return IMAGES.chocolate;
    if (/strawberry|blueberry|mango|kiwi|lychee/i.test(item.name)) return IMAGES.fruit;
    return IMAGES.thai;
  }
  if (item.category === 'Food') return /indomie/i.test(item.name) ? IMAGES.noodles : IMAGES.rice;
  if (item.category === 'Snacks') return /fries/i.test(item.name) ? IMAGES.fries : IMAGES.dimsum;
  return IMAGES.cake;
}
...
export function renderIndex() {
  ...
  const frontendMenu = MENU.map((item) => ({
    ...item,
    image: productImage(item),
    note: NOTES[item.category],
  }));

  const categoryTabs = categories
    .map((category) => `<button type="button" data-category="${esc(category)}">${esc(category)}</button>`)
    .join('');

  const cards = frontendMenu.map((item) => `
    <article class="menu-card" data-id="${esc(item.id)}" data-category="${esc(item.category)}" data-name="${esc(item.name.toLowerCase())}">
      <div class="menu-image">
        <img src="${esc(item.image)}" alt="Foto ${esc(item.name)}" loading="lazy">
        ${item.favorite ? '<span class="favorite">★ Favorit</span>' : ''}
        <small>${esc(item.category)} • Harumi</small>
      </div>
      <div class="menu-copy">
        <span>${esc(item.category)}</span>
        <h3>${esc(item.name)}</h3>
        <p>${esc(item.note)}</p>
        <div><strong>${item.large ? 'Mulai ' : ''}Rp ${formatRupiah(item.regular)}</strong><button type="button" class="add-product" data-id="${esc(item.id)}" aria-label="Tambah ${esc(item.name)}">+</button></div>
      </div>
    </article>`).join('');

  const harumiJson = JSON.stringify({
    menu: frontendMenu,
...
<title>${esc(CONFIG.name)} - ${esc(CONFIG.tagline)}</title>
...
<main>
  ...
  <section class="location" id="location">
    ...
    <div><span>03</span><strong>Everyday comfort</strong><small>${esc(CONFIG.openHours)}</small></div>
  </section>
</main>

<footer><a class="brand" href="#home"><span>Harumi</span><small>はるみ</small></a><div><strong>Outlet</strong><p>${esc(CONFIG.address)}</p></div><div><strong>Cloudflare Workers</strong><p>Edge-rendered • KV storage • Git ready</p></div></footer>
